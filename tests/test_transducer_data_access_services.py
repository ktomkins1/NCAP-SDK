...
class TestTransducerDataAccessServices(unittest.TestCase):
    ...
    def test_read_transducer_block_data_from_multiple_channels(self):
        """ Test reading transducer block data from multiple channels. """
    ...
    def test_write_transducer_block_data_to_a_channel_of_a_tim(self):
        """ Test writing transducer block data to a channel of a tim """
        ...
        request = {
                ...
                'sample_interval': .1,
                'start_time': .1,
                'data_values': [1024, 1025, 1026]
                }

        expected_response = (0, 1234, 1, 2)
        expected_output = [1024, 1025, 1026]

        response = tdas.write_transducer_block_data_to_a_channel_of_a_tim(
                                                request['ncap_id'],
                                                request['tim_id'],
                                                request['channel_id'],
                                                request['timeout'],
        ...
        tdaccs.open.assert_called_with(01, 02)
        tdaccs.write_data.assert_called_with(1, 100, 5, mock.ANY)
        self.assertEqual(expected_response, response)
        self.assertEqual(expected_output, self.result)
